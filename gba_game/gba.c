...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  //L17 ppt slide 25 if breaks (memcpy not TRANSFER)
  //make local variables volatile
  for(int i = 0; i < height; i++)
  {
      DMA[3].src = &color;
      DMA[3].dst = &videoBuffer[(row*WIDTH)+(col + i)];
      DMA[3].cnt = width | DMA_SOURCE_FIXED | DMA_ON;
  }
}

void drawFullScreenImageDMA(const u16 *image) {
  //see L17 ppt slide 25 if breaks
  DMA[3].src = image;
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  //L17 ppt slide 25 if breaks (DMA_TRANSFER)
  //if breaks try other channels
  for(int i = 0; i < height; i++)
  {
...
void fillScreenDMA(volatile u16 color) {
  // same thing as fullscreen image
  DMA[3].src = &color;
	DMA[3].dst = videoBuffer;
